...
impl Pass for SymtabDebugPass {
    ...
    fn when_finish_or_panic(&mut self, ctx:&mut crate::toolkit::context::NhwcCtx) {
        if self.is_gen_png {
            ...
            generate_png_by_graph_multi_tasks(&symtab_g.clone(), "symtab".to_string(), &[Config::Record, Config::Rounded, Config::SymTab, Config::Title("symtab_graph".to_string())],&mut ctx.io_task_list).unwrap();
        }
    }
}
