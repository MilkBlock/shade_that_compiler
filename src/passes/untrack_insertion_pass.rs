...
impl Pass for UntrackInsertionPass {
    ...
    fn when_finish_or_panic(&mut self, ctx:&mut crate::toolkit::context::NhwcCtx) {
        if self.is_gen_ssa_cfg{
            ...
            generate_png_by_graph_multi_tasks(&ctx.cfg_graph.clone(), "untrack_cfg".to_string(), &[Config::Record, Config::Rounded, Config::Title("ssa_cfg".to_string()), Config::NodeIndexLabel, Config::CfgBlock],&mut ctx.io_task_list).unwrap();
        }
        if self.is_gen_symtab {
            ...
            generate_png_by_graph_multi_tasks(&ctx.symtab_graph.clone(), "symtab".to_string(), &[Config::Record, Config::Rounded, Config::Title("ssa_symtab".to_string()), Config::NodeIndexLabel, Config::CfgBlock],&mut ctx.io_task_list).unwrap();
        }
    }
}
