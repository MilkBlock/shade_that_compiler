   ...
   // 运行这个pass
    ...
    fn when_finish_or_panic(&mut self, ctx:&mut crate::toolkit::context::NhwcCtx) {
        if self.is_gen_ssa_cfg_png{
            ...
            generate_png_by_graph_multi_tasks(&ctx.cfg_graph.clone(), "ssa_deconstruction_cfg".to_string(), &[Config::Record, Config::Rounded, Config::Title("ssa_cfg".to_string()), Config::NodeIndexLabel, Config::CfgBlock],&mut ctx.io_task_list).unwrap();
        }
        if self.is_gen_symtab_graph_png {
            ...
            generate_png_by_graph_multi_tasks(&ctx.symtab_graph.clone(), "ssa_deconstruc_symtab".to_string(), &[Config::Record, Config::Rounded, Config::Title("ssa_symtab".to_string()), Config::NodeIndexLabel, Config::CfgBlock],&mut ctx.io_task_list).unwrap();
        }
    }
    // 返回pass的名称
