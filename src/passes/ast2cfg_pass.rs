...
impl Pass for Ast2CfgPass {
    ...
    fn when_finish_or_panic(&mut self, ctx:&mut crate::toolkit::context::NhwcCtx) {
        if self.is_gen_png {
            ...
            generate_png_by_graph_multi_tasks(&ctx.cfg_graph.clone(), "cfg_graph".to_string(), &[Config::Record, Config::Rounded,Config::CfgBlock, Config::Title("cfg_graph".to_string()), Config::NodeIndexLabel],&mut ctx.io_task_list).unwrap();
        }
    }
}
